      ...
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        ...
        <div className="flex items-center gap-8">
          <Link href="/dashboard" className="flex items-center gap-2">
            <div className="w-8 h-8 bg-zinc-900 rounded-lg flex items-center justify-center text-white font-bold uppercase">M</div>
        ...
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <div className="relative">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="flex items-center gap-3 focus:outline-none">
            ...
            <AnimatePresence>
              {isMenuOpen && (
                <>
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 top-12 w-56 bg-white rounded-xl shadow-xl border border-zinc-200 z-50 overflow-hidden">
...
function NavLink({ href, icon, label, active }: { href: string; icon: any; label: string; active: boolean }) {
  return (
    <Link href={href} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-2 ${active ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-900'}`}>
      {icon} {label}
    </Link>
  );
}
