...
export default function Generator() {
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      ...
      // If Logged In: Fetch Sequential Number and Profile
      if (user) {
        try {
  ...
  const initiateAction = (action: () => void) => {
    if (!user) {
      if (confirm("Sign up for a free account to Download or Share receipts?")) {
        router.push('/login');
      }
      return;
    }
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center">
            ...
            <p className="text-sm text-zinc-500 mb-6">Are you sure the amounts and names are correct? This will be saved to your history.</p>
      ...
      <div className="flex-1 relative overflow-hidden flex flex-col md:flex-row">
        ...
        <div className={`flex-1 h-full overflow-y-auto bg-zinc-50 p-4 md:p-6 space-y-6 ${activeTab === 'preview' ? 'hidden md:block' : 'block'}`}>
          <div className="space-y-6 pb-40 md:pb-10 max-w-2xl mx-auto">
            ...
            <section className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
              <div className="flex justify-between items-center"><h3 className="font-bold text-xs text-zinc-500 uppercase">Items Purchased</h3><button onClick={addItem} className="text-xs font-bold text-zinc-900 hover:bg-zinc-100 px-3 py-1 rounded-full">+ Add Item</button></div>
              {data.items.map((item) => (
            ...
            <section className="bg-white p-5 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
              <h3 className="font-bold text-xs text-zinc-500 uppercase">Totals & Method</h3>
              <div className="grid grid-cols-2 gap-4">
        ...
        <div className={`flex-1 h-full bg-zinc-200/50 flex flex-col relative ${activeTab === 'edit' ? 'hidden md:flex' : 'flex'}`}>
          <div className="bg-white/80 backdrop-blur-md border-b border-zinc-200 p-3 flex justify-between items-center z-10 shadow-sm shrink-0">
             ...
             <div className="flex gap-2">
               <button onClick={() => setSettings({...settings, showLogo: !settings.showLogo})} className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${settings.showLogo ? 'bg-zinc-100' : 'text-zinc-400'}`}>Logo</button>
               <button onClick={() => setSettings({...settings, template: settings.template === 'simple' ? 'detailed' : 'simple'})} className="px-3 py-1.5 rounded-full text-xs font-bold bg-zinc-100 shadow-sm border border-zinc-100 text-zinc-700">{settings.template === 'simple' ? 'Simple' : 'Detailed'}</button>
             </div>
          </div>

          <div className="flex-1 overflow-auto flex items-center justify-center p-4 bg-zinc-100/50 relative">
             {/* ANTI-SCREENSHOT WATERMARK - ONLY FOR GUESTS */}
             {!user && (
        ...
        {/* MOBILE TABS */}
        <div className="md:hidden absolute bottom-0 left-0 right-0 bg-white border-t border-zinc-200 flex z-40 pb-safe">
